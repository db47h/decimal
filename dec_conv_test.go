...
func TestDecString(t *testing.T) {
	...
	for _, a := range decStrTests {
		s := string(a.x.utoa(a.b))
		if s != a.s {
			t.Errorf("string%+v\n\tgot s = %s; want %s", a, s, a.s)
		}

		x, b, _, err := dec(nil).scan(strings.NewReader(a.s), a.b, false)
		if x.cmp(a.x) != 0 {
...
var decScanTests = []struct {
	...
	{"123_.", 0, true, dec{123}, 10, 0, errInvalSep, 0},
	{"._123", 0, true, dec{123}, 10, -3, errInvalSep, 0},
	{"0b__1000", 0, false, dec{0x8}, 2, 4, errInvalSep, 0},
	{"0o60___0", 0, false, dec{0600}, 8, 3, errInvalSep, 0},
	{"0466_", 0, false, dec{0466}, 8, 3, errInvalSep, 0},
	{"01234567_8", 0, false, dec{01234567}, 8, 7, errInvalSep, '8'},
	{"1_.", 0, true, dec{1}, 10, 0, errInvalSep, 0},
	{"0._1", 0, true, dec{1}, 10, -1, errInvalSep, 0},
	{"2.7_", 0, true, dec{27}, 10, -1, errInvalSep, 0},
	{"0x1.0_", 0, true, dec{0x10}, 16, -1, errInvalSep, 0},

	// valid: separators are not accepted for base != 0
	{"0_", 10, false, nil, 10, 1, nil, '_'},
	{"1__0", 10, false, dec{1}, 10, 1, nil, '_'},
	{"0__8", 10, false, nil, 10, 1, nil, '_'},
	{"xy_z_", 36, false, dec{33*36 + 34}, 36, 2, nil, '_'},
	...
	{"0b1", 0, false, dec{1}, 2, 1, nil, 0},
	{"0B11000101", 0, false, dec{0xc5}, 2, 8, nil, 0},
	{"0B110001012", 0, false, dec{0xc5}, 2, 8, nil, '2'},
	{"07", 0, false, dec{7}, 8, 1, nil, 0},
	{"08", 0, false, nil, 10, 1, nil, '8'},
	{"08", 10, false, dec{8}, 10, 2, nil, 0},
	{"018", 0, false, dec{1}, 8, 1, nil, '8'},
	{"0o7", 0, false, dec{7}, 8, 1, nil, 0},
	{"0o18", 0, false, dec{1}, 8, 1, nil, '8'},
	...
	{"0.12", 2, true, dec{1}, 2, -1, nil, '2'},
	{"0b0.1", 0, true, dec{1}, 2, -1, nil, 0},
	...
	{"0x_f0_0d", 0, false, dec{0xf00d}, 16, 4, nil, 0},
	...
	{"0x_f00d.1E2", 0, true, dec{0xf00d1e2}, 16, -3, nil, 0},
	{"0x_f00d.1eg", 0, true, dec{0xf00d1e}, 16, -2, nil, 'g'},
}

func TestDecScanBase(t *testing.T) {
	for _, a := range decScanTests {
		r := strings.NewReader(a.s)
		x, b, count, err := dec(nil).scan(r, a.base, a.frac)
		if err != a.err {
...
func TestDecScanPi(t *testing.T) {
	var x dec
...
func TestDecScanPiParallel(t *testing.T) {
	const n = 2
	c := make(chan int)
	for i := 0; i < n; i++ {
		go func() {
...
func BenchmarkDecStringPiParallel(b *testing.B) {
	var x dec
